...
If `npm view clone-to-green version` already returns `0.1.2`, do not republish.

If npm latest is `0.1.1`, publish `0.1.2`.
...
3. If the current version is already `0.1.2`, stop.
4. If the current version is `0.1.1`, run `npm pack --dry-run`.
